...
/**
 * Solo el agrupado de miles sale de `Intl`; el símbolo lo ponemos nosotros.
 *
 * Pedirle a `Intl` el formato de moneda completo devuelve `$` + **U+00A0**
 * (espacio duro) + la cifra, y ese carácter invisible depende de la versión
 * de ICU: la de Node y la del navegador no tienen por qué coincidir. Como
 * estos importes se pintan dentro de componentes cliente, una diferencia ahí
 * rompe la hidratación —ya pasó con las fechas del panel de pago— y React
 * tira el HTML del servidor para volver a pintarlo.
 *
 * El agrupado de miles, en cambio, es estable entre versiones.
 */
