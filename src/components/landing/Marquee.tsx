 ...
 * sección de la rúbrica. Un lector de pantalla no tiene por qué oír «nicho
 * cuarenta» doce veces.
 */
export function Marquee({ items }: MarqueeProps) {
  ...
  return (
    <div
      aria-hidden="true"
      className="overflow-hidden border-y-2 border-accent bg-accent py-3.5"
    >
        ...
        {[0, 1].map((copia) => (
          <div key={copia} className="flex shrink-0">
            {tramo.map((item, i) => (
              <span
                key={`${copia}-${i}`}
                className="display flex items-center gap-8 whitespace-nowrap px-4 text-xl text-print sm:text-2xl"
              >
                {item}
